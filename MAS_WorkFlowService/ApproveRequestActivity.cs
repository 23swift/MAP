...
namespace MAS_WorkFlowService
{
    ...
    public sealed class ApproveRequestActivity : CodeActivity
    {
        ...
        protected override void Execute(CodeActivityContext context)
        {
            ...
            using (var unitOfWork = new UnitOfWork())
            {
                ...
                var currentRequest = unitOfWork.RequestRepository.Get(_requestId,x=>x.RequiredApprovals);
                
                var approval = currentRequest.RequiredApprovals.Where(x => x.User == null).FirstOrDefault();
                ...
                if (currentRequest.RequiredApprovals.Where(x => x.User == null).FirstOrDefault() == null)
                {

                    context.SetValue(this.hasCompleteApproval, true);

                    if (currentRequest.RequiredApprovals.Where(x => x.isApproved.Equals(false)).FirstOrDefault() == null)
                    {
